...
use App\Models\KnowledgeBase; // Pasien BOLEH baca ini
use Carbon\Carbon;
...
class UserChatbotController extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        try {
            ...
            // ==========================================================
            // TAHAP 1: CARI DI KNOWLEDGE BASE (RAG UNTUK PASIEN)
            // ==========================================================
            ...
            if ($embedResponse->successful()) {
                ...
                foreach ($kbs as $kb) {
                    ...
                    if ($kbVector) {
                        ...
                        if ($similarity > 0.12) { // Ambil yang relevan
                            $matches[] = ['jawaban' => $kb->jawaban, 'kat' => $kb->kategori, 'score' => $similarity];
                        }
                    }
                }
            ...
            // ==========================================================
            // TAHAP 2: PROMPT ENGINEERING (LEBIH FLEXIBEL)
            // ==========================================================
            ...
            - Layanan: Poli Umum dan Poli Gigi.

            [INSTRUKSI]:
            1. Jawablah pertanyaan pasien berdasarkan [DATA DASAR KLINIK] dan [INFORMASI TAMBAHAN] di bawah.
            2. Jika pertanyaan mengandung beberapa hal (misal: alamat dan layanan), jawab SEMUANYA.
            3. JANGAN PERNAH memberikan info Stok Obat, Nama Pasien lain, Riwayat Medis, atau Keuangan Klinik.
            4. Jika ditanya hal terlarang tersebut, katakan: 'Mohon maaf, saya tidak memiliki akses ke database medis demi menjaga privasi. Silakan hubungi admin di (0361) 410764.'
            5. Gunakan format HTML (<b>, <br>) agar rapi.

            [INFORMASI TAMBAHAN]:\n" . $contextData;

            // ==========================================================
            // TAHAP 3: GENERATION (FIX MODEL VERSION 2.5 FLASH)
            // ==========================================================
            $chatHistory = session()->get('user_chatbot_memory', []);
            $geminiContents = [
                ['role' => 'user', 'parts' => [['text' => $systemPrompt]]],
                ['role' => 'model', 'parts' => [['text' => 'Baik, saya siap melayani pasien dengan informasi yang tersedia.']]]
            ];

            foreach ($chatHistory as $history) {
                $geminiContents[] = ['role' => $history['role'], 'parts' => [['text' => $history['text']]]];
            }
            $geminiContents[] = ['role' => 'user', 'parts' => [['text' => $userMessage]]];
            ...
            if ($generateResponse->successful()) {
                ...
                return response()->json(['reply' => str_replace(['**', '*'], ['<b>', '</b>'], nl2br($botReplyRaw))]);
            }

            return response()->json(['reply' => '⚠️ Gagal terhubung ke AI.']);

        } catch (\Exception $e) {
            return response()->json(['reply' => '⚠️ Kesalahan Sistem: ' . $e->getMessage()]);
        }
    }
